    ...
    private static String isPatternExist(int[][] matrix, int[][] pattern) {
        ...
        for (int i = 0; i < matrix.length; i++) {
            for (int j = mStart; j < matrix[0].length; j++) {
                if (prow < pattern.length && pcol < pattern[0].length && matrix[i][j] != pattern[prow][pcol]) {
                    if (mStart != 0) {
                        j = 0;
                        mStart = 0;
                    ...
                    prow = 0;
                    pcol = 0;
                } else if (prow < pattern.length && pcol < pattern[0].length && matrix[i][j] == pattern[prow][pcol]) {
                    pcol++;
