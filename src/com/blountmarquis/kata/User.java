...
public class User {
    ...
    public User() {this.rank = DEFAULT_RANK;}
    ...
    public void incProgress(int rankCompleted) {
        if(rankCompleted == 0 || rankCompleted < -8 || rankCompleted > 8) throw new IllegalArgumentException();
        ...
        if(this.progress >= 100) this.progress %= PROGRESS_POINT_TARGET;
        if(this.rank == MAX_RANK) this.progress = 0;
    }
    ...
    private int calculateProgress(int rank, int progress, int rankCompleted) {
        ...
        } else if (isExceriseOneLevelLowerThanRank(rankCompleted, rank)) progress += ONE_LEVEL_LOWER_POINTS;
        else if (isExcerciseLevelGreaterThanRank(rankCompleted)) {
            int difference = Math.abs(rank - rankCompleted);
            if(isPolarityChanged(rank,rankCompleted)) difference--;
            progress += POINT_ACCELERATE_CONSTANT * (difference * difference);
        }
        return progress;
    }

    private boolean isExceriseAndRankSameLevel(int rankCompleted) {return rankCompleted == rank;}

    private boolean isExceriseOneLevelLowerThanRank(int rankCompleted, int rank) {
        if(isPolarityChanged(rankCompleted, rank)) rankCompleted++;
        return rankCompleted + 1 == rank;
    }

    private boolean isExcerciseLevelGreaterThanRank(int rankCompleted) {return rankCompleted > rank;}
    ...
    private int ifEnoughPointsEarnedCycleProgressRankUp(int rank, int progress) {
        if (progress >= PROGRESS_POINT_TARGET) {
            ...
            int temp = rank;
            rank += cycles;
            if (isPolarityChanged(temp,rank)) rank++;
            if (rank >= MAX_RANK) {
                rank = MAX_RANK;
            }
        }
    ...
    private boolean isPolarityChanged(int prevRank, int newRank) {return prevRank < 0 && newRank >= 0;}
