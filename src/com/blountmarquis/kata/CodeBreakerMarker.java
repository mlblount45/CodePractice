...
public class CodeBreakerMarker {
  ...
  private static String buildMarker(String secret, String guess) {
    ...
    for (int i = 0; i < secret.length(); i++) {
      if (isGuessIndexMatchSecret(secret, guess, i)) {
        sbMatch.append("b");
      } else {
        if (isGuessContainedInSecret(secret, guess, i)) {
          sbContain.append("w");
        }
      }
    }
  ...
  private static boolean isGuessContainedInSecret(String secret, String guess, int index) {
    return secret.indexOf(guess.charAt(index)) > -1;
  }

  private static boolean isGuessIndexMatchSecret(String secret, String guess, int index) {
    return secret.charAt(index) == guess.charAt(index);
  }
}
