...
public class BinaryTree<T extends Comparable<T>> {
    ...
    public Node<T> successor(Node nodeOfInterest){
        if (nodeOfInterest == null) return nodeOfInterest;
    ...
    public Node<T> predecessor(Node nodeOfInterest){
        if (nodeOfInterest == null) return null;
    ...
    private static <E extends Comparable<E>> boolean isBST(Node<E> n, E min, E max){
        ...
        if((min != null && min.compareTo(n.value) > 0) || (max != null && max.compareTo(n.value) <=0))
            return false;
    ...
    private static <T extends Comparable<T>> void constructTreeFromInOrderList(BinaryTree tree, T[] arr, int start, int end) {
        if(start >= end) return;
    ...
    private int getSumOfHeight(Node n, int height) {
        if(height == 0)return (Integer)n.value;
