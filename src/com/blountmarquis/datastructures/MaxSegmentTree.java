...
public class MaxSegmentTree {
    ...
    private void update(Node node, int i, int val) {
        if(node.start == node.end) node.max = val;
        else{
    ...
    private int maxRange(Node node, int i, int j) {
        ...
        int mid = node.start + (node.end + node.start) / 2;
        if (i >= mid + 1) return maxRange(node.right, i, j);
        else if (j <= mid) return maxRange(node.left, i, j);
        else return Math.max(maxRange(node.left, i, mid), maxRange(node.right, mid + 1, j));
    }
