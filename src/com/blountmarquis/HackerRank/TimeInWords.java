...
public class TimeInWords {
    ...
    private static Map<Integer,String>  hoursLookup = new HashMap<Integer, String>(){{
        put(1, "one");put(2,"two");put(3,"three");put(4,"four");put(5,"five");put(6,"six");put(7,"seven");put(8,"eight");
    ...
    private static Map<Integer, String> specialLookup = new HashMap<Integer, String>() {{
        put(0, "o' clock"); put(15, "quarter"); put(30, "half");put(45, "quarter");
    }};
    ...
    private static String getTime(int hour, int min) {
        if(min == 0) return hoursLookup.get(hour) + " " + specialLookup.get(min);
        if(min == 15 || min == 30) return specialLookup.get(min) + " past " + hoursLookup.get(hour);
        if(min == 45) return specialLookup.get(min) + " to " + (hour == 12 ? hoursLookup.get(1) : hoursLookup.get(hour+1));
        ...
        if(min < 30) {
            ...
            return minInWords + (min == 1 ? " minute " : " minutes ") + "to " + (hour == 12 ? hoursLookup.get(1) : hoursLookup.get(hour+1));
        }

    }

    private static String intToWords(int min) {
        if(onesLookup.containsKey(min)) return onesLookup.get(min);
        else if(teensLookup.containsKey(min)) return teensLookup.get(min);
        else if(tensLookup.containsKey(min)) return tensLookup.get(min);
        else return tensLookup.get(min / 10) + " " + onesLookup.get(min % 10);
    }
}
