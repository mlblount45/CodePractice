...
public class JavaBitSet {
    ...
    private static void manipulateBits(Scanner in, BitSet bitSet1, BitSet bitSet2) {
        ...
        if (isAndOrXorOperand(operand)) {
            ...
            else andOrXorOperationBitManipulation(operand,bitSet2 ,bitSet1 );
        }else{
            if(leftOperand == 1)
                flipSetBitManipulation(operand,rightOperand,bitSet1);
            else flipSetBitManipulation(operand,rightOperand,bitSet2);
        }
    }
    ...
    private static void andOrXorOperationBitManipulation(String operand, BitSet leftOperand, BitSet rightOperand) {
        switch (operand) {
            case "AND":{
