...
public class MatrixPath {
  ...
  public static boolean isPathExist(int[][] matrix, Position start, Position end) {
    ...
    if (start.row < 0 || start.row >= matrix.length) {
      return false;
    }
    if (start.col < 0 || start.col >= matrix[0].length) {
      return false;
    }
  ...
  private static Position moveDown(int[][] matrix, Position location) {
    ...
    if (move >= matrix.length || matrix[move][keep] != 0) {
      return null;
    }
  ...
  private static Position moveRight(int[][] matrix, Position location) {
    int keep = location.row;
    int move = location.col + 1;
    if (move >= matrix[0].length || matrix[keep][move] != 0) {
      return null;
    }
    return new Position(keep, move);
  }
  ...
  public static void main(String[] args) {
    ...
    System.out
        .println("is path exist? : " + MatrixPath.isPathExist(matrix, start, end)); //output true
    start = new Position(0, 0);
    end = new Position(2, 1);
    System.out
        .println("is path exist? : " + MatrixPath.isPathExist(matrix, start, end)); //output false
