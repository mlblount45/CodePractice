...
public class UnStringifyTest {
	...
	public void standingStringToConvertAsNeeded() {
		assertStringIsUnStringified("select max(timestamp) as lastDetect, count(timestamp) as detectCount " +
				"from write " +
				...
				"ORDER BY timestamp desc limit 1 ", "select max(timestamp) as lastDetect, count(timestamp) as detectCount " +
				"from write " +
				"where docType is not missing " +
				"AND docType = 'beaconDetectRecord' " +
				"AND beacon.proximityUUID = $uuid " +
